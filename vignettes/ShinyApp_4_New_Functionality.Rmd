...
The values of input design elements are all passed into a user function as (de-reacted) named list.  The following table,
which is not comprehensive, gives the most relevant items:
...
* Produces a `dygraph()` with the an average rolling correlation, as well as 25th and 75th percentiles
...
Doing so will save the definition in the disk cache, so we just need to rerun  `av_runShiny()` and then we can run it!
