...
[FRED](https://fred.stlouisfed.org/)) and give it our own ticker
`HH_GAS`. First we download the price series and get the columns we
need. Then we add some basic description, including most critically the
asset type, so the app knows where to get data going forward.
...
directly from [FRED](https://fred.stlouisfed.org/), let’s add FEDFUNDS
as its own ticker:
